        ...
        for i in product_in_stock:
            if amount > 0:
                ...
                if amount >= i['amount']:
                    ...
                    sale = Sales(sale_id = last_sale_id, stock_id = stock_discharge, product_id = sold_product, amount = i['amount'], price = price, user_id = request.user.id)
                    sale.save()
                    amount -= i['amount']
                    ...
                    last_sale_id = last_sale.sale_id + 1
                    sold_product = get_object_or_404(Products, pk = sold_product_id)
                    sale = Sales(sale_id = last_sale_id, stock_id = stock_discharge, product_id = sold_product, amount = amount, price = price, user_id = request.user.id)
                    sale.save()
                    amount -= amount
                elif amount == 0:
                    break
